...
class Cliconf(CliconfBase):
    ...
    def get_device_operations(self):
        return {                                    # supported: ---------------
            ...
            'supports_onbox_diff': True,            # identify if on box diff capability is supported or not
                                                    # unsupported: -------------
            'supports_rollback': False,             # no rollback
    ...
    def edit_config(self, candidate=None, commit=True, replace=None, comment=None):
        ...
        match = re.search(r'^\*', to_text(prompt, errors='surrogate_or_strict').strip())
        if match:
            return {'request': requests, 'response': responses, 'diff': match.group(0)}
        else:
